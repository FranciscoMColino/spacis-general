...
class GCSApp:
    ...
    def create_server_status_widget(self):
        ...
        self.server_status_label = tk.Label(
            server_frame, text="Waiting for client...")
        self.server_status_label.grid(row=1, column=0, pady=5)
        self.client_last_update_label = tk.Label(
            server_frame, text="Last update: ---")
    ...
    def create_system_control_widget(self):
        ...
        system_control_frame.grid(
            row=1, column=0, padx=10, pady=10, columnspan=2)
        tk.Label(system_control_frame, text="Commands").grid(
            row=0, column=0, columnspan=3)

        self.override_status_label = tk.Label(
            system_control_frame, text="Override Status: OFF")
        self.override_status_label.grid(row=1, column=0, pady=5)
        self.override_toggle = tk.Button(
            system_control_frame, text="Turn ON", command=self.toggle_override)
        self.override_toggle.grid(row=1, column=1, pady=5, padx=5)

        tk.Label(system_control_frame, text="Clock on boot:").grid(
            row=2, column=0, pady=5)

        self.clock_on_boot_label = tk.Label(system_control_frame, text="--")
        self.clock_on_boot_label.grid(row=2, column=1, pady=5)

        tk.Label(system_control_frame, text="CPU clock:").grid(
        ...
        tk.Label(system_control_frame, text="Config clock:").grid(
            row=4, column=0, pady=5)

        self.config_clock_label = tk.Label(system_control_frame, text="--")
        self.config_clock_label.grid(row=4, column=1, pady=5)

        tk.Button(system_control_frame, text="Config CPU clock (MHz)",
                  command=self.set_cpu_clock).grid(row=5, column=0, pady=5, padx=5)
        self.system_control_data.config_cpu_clock = tk.IntVar()
        self.cpu_clock_slider = tk.Scale(system_control_frame, from_=600, to=2200,
                                         variable=self.system_control_data.config_cpu_clock, orient=tk.HORIZONTAL, resolution=100)
    ...
    def create_temperature_control_widget(self):
        ...
        tk.Label(sensor_frame, text="Temperature control").grid(
            row=0, column=0, columnspan=2)

        box_temp_frame = tk.Frame(sensor_frame, bd=1, relief=tk.FLAT)
        box_temp_frame.grid(row=1, column=0, pady=5, padx=5)
        tk.Label(box_temp_frame, text="Box",
                 font='Segoe 9 bold').grid(row=0, column=0)

        box_temperature_frame = tk.Frame(
            box_temp_frame, bd=1, relief=tk.GROOVE)
        box_temperature_frame.grid(row=1, column=0, pady=5, padx=5)
        tk.Label(box_temperature_frame, text="Box temp:", width=8,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        self.box_temperature_label = tk.Label(
            box_temperature_frame, text="--", width=5, anchor=tk.W)
        ...
        tk.Label(box_fan_frame, text="Box fan:", width=8,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        self.box_fan_label = tk.Label(box_fan_frame, text="--", width=4)
        self.box_fan_label.grid(row=0, column=1, pady=5, padx=3.25)

        self.box_fan_toggle = tk.Button(
            box_temp_frame, text="Turn ON", command=self.toggle_box_fans)
        ...
        tk.Label(rpi_temp_frame, text="RPi",
                 font='Segoe 9 bold').grid(row=0, column=0)

        cpu_temperature_frame = tk.Frame(
            rpi_temp_frame, bd=1, relief=tk.GROOVE)
        cpu_temperature_frame.grid(row=1, column=0, pady=5, padx=5)
        tk.Label(cpu_temperature_frame, text="CPU temp:").grid(
            row=0, column=0, pady=5)
        self.cpu_temperature_label = tk.Label(
            cpu_temperature_frame, text="--", width=5, anchor=tk.W)
        ...
        tk.Label(rpi_fan_frame, text="RPi fan:", width=8,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        self.rpi_fan_label = tk.Label(rpi_fan_frame, text="--", width=4)
        self.rpi_fan_label.grid(row=0, column=1, pady=5, padx=3.25)

        self.rpi_fan_toggle = tk.Button(
            rpi_temp_frame, text="Turn ON", command=self.toggle_rpi_fans)
    ...
    def create_gps_widget(self):
        ...
        tk.Label(gps_longitude_frame, text="Lon:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        self.gps_longitude_label = tk.Label(
            gps_longitude_frame, width=10, anchor=tk.W)
        ...
        tk.Label(gps_altitude_frame, text="Alt:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        self.gps_altitude_label = tk.Label(
            gps_altitude_frame, width=10, anchor=tk.W)
        ...
        tk.Label(gps_track_frame, text="Track:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        ...
        tk.Label(gps_speed_frame, text="Speed:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        ...
        tk.Label(gps_climb_frame, text="Climb:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
        ...
        tk.Label(gps_time_frame, text="Time:", width=5,
                 anchor=tk.W).grid(row=0, column=0, pady=5)
    ...
    def create_received_sequence_status_widget(self):
        ...
        self.rcv_data_label = tk.Label(
            raw_data_frame, text="0", width=45, anchor=tk.W, relief=tk.SUNKEN, bg="white")
        ...
        self.received_freq_label = tk.Label(
            date_time_frame, text="Received freq", width=10, anchor=tk.E)
        self.received_freq_label.grid(row=0, column=3, pady=5)

        self.received_freq_text = tk.Label(
            date_time_frame, text="0", width=5, anchor=tk.E, relief=tk.SUNKEN, bg="white")
        self.received_freq_text.grid(row=0, column=4, pady=5, padx=5)

        tk.Label(date_time_frame, text="Time:", anchor=tk.W).grid(
            row=0, column=0, pady=2)

        self.received_time_label = tk.Label(
            date_time_frame, text='-----', anchor=tk.W)
    ...
    def draw_spectogram(self):
        ...
        if self.data_viz_control.selected_spec_mode.get().startswith("Sensor"):
            ...
            for i in range(0, 4):
                ...
                ax.clear()
                ax.title.set_text("Sensor " + str(i))
                ax.specgram(display_data[i],
                            Fs=SAMPLE_FREQ, vmin=vmin, vmax=vmax)
                ax.set_ylim(ylim)

            self.spectogram_window.draw()

        self.performance_recordings.record(
            "update_spectogram", time.time() - PM_START)
    ...
    def update_server_status(self):
        ...
        self.client_last_update_label.config(
            text="Last update: " + self.server.client.last_update)
    ...
    def update_data(self, data):
        ...
        self.received_freq_text.config(
            text=self.server.average_batch_freq)

        self.received_time_label.config(
            text=datetime.now().strftime("%H:%M:%S"))

        self.performance_recordings.record(
            "update_sensor_data", time.time()-PM_START)

    def update_temperature_status_frame(self):
        # Function to update temperature label
        self.box_temperature_label.config(
            text=str(self.temperature_status.box_temperature))
        self.cpu_temperature_label.config(
            text=str(self.temperature_status.cpu_temperature))

        self.box_fan_label.config(text="ON" if self.temperature_status.box_fan else "OFF",
                                  bg="green" if self.temperature_status.box_fan else "red")
        self.rpi_fan_label.config(text="ON" if self.temperature_status.rpi_fan else "OFF",
                                  bg="green" if self.temperature_status.rpi_fan else "red")
    ...
    def update_system_control(self):
        ...
        self.override_status_label.config(
            text="Override Status: " + ("ON" if self.system_control_data.override_mode else "OFF"))

        self.clock_on_boot_label.config(
            text=self.system_control_data.clock_on_boot)

        self.cpu_clock_label.config(
            text=self.system_control_data.current_cpu_clock)

        self.config_clock_label.config(
            text=self.system_control_data.clock_config)
    ...
    def set_temperature_status(self, data):
        self.temperature_status.cpu_temperature = round(
            data["cpu_temperature"], 2)
        self.temperature_status.box_temperature = round(
            data["box_temperature"], 2)
