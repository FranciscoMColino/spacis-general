...
class DataRecorder:
    ...
    def record_sensor_data(self, data, elapsed, pps_id):
        transformed_data = [str(x) for x in data]
        transformed_data.extend([str(elapsed), str(pps_id)])
        self.sensor_record_file.write(','.join(transformed_data) + '\n')

    def record_multiple_sensor_data(self, data, elapsed, pps_id):
        # self.local_data.extend(data)
        for line in data:
            self.record_sensor_data(line, elapsed.pop(0), pps_id.pop(0))
