...
unsigned int time_safe = 3;        // time before next transmission where no action is taken
...
void setup()
{
  ...
  Serial.println("SETUP: Generating sequence");
...
String formatWithCommas(unsigned long num) {
    ...
    if (num>pow(10, 6)){
      char temp_str[9];
      sprintf(temp_str, "%lu,", num/1000000);
      p = mystrcat(p, temp_str);
    }
    if (num>pow(10, 3)){
      char temp_str[9];
      sprintf(temp_str, "%lu,", (num/1000)%1000);
      p = mystrcat(p, temp_str);
    }
    char temp_str[9];
    sprintf(temp_str, "%lu", num%1000);
    p = mystrcat(p, temp_str);
    return num_string;
}
...
void loop()
{
  ...
  case START_S:
    ...
    Serial.println(formatWithCommas(micros()));
    
    acc_jitter = micros() % (time_between_tx * 1000000);
    Serial.print("LOG: acc_jitter ");
    Serial.println(formatWithCommas(acc_jitter));
  ...
  case TIMER1_FINISHED:
    printFreeMemory();
    Serial.print("ELAPSED: TIMER1_FINISHED ");
    Serial.println(formatWithCommas(elapsed));
  ...
  case DATA_TRANSFER:
    if (elapsed >= (time_between_tx - time_safe) * (1000000))
    {
      ...
      Serial.print("ELAPSED: DATA_TRANSFER: ");
      Serial.println(formatWithCommas(elapsed));
    }
    else
    {
      ...
      if (Serial.available())
      {
        ...
        { // Check if it's the expected message type (starting with 'A')
          message.remove(0, 1);

          int index = 0;
          char *ptr = strtok(const_cast<char *>(message.c_str()), " ");
          Serial.print("DELAYS_RCV:");
          while (ptr != nullptr && index < 6)
          {
            
            delay_offsets[index] = atoi(ptr); // Convert string to integer
            ptr = strtok(nullptr, " ");
            
            char delay_str[5];
            sprintf(delay_str, " %d", delay_offsets[index]);

            index++;

            Serial.print(delay_str);
          }
          Serial.println();
        }
      }
  ...
  case SEQUENCE_CYCLE:
    ...
    Serial.print("DELAY_USE: ");
    for (int i = 0; i < 6; i++)
    {
      Serial.print(delay_offsets[i]); // Print the value
      Serial.print(" ");              // Print a space
    }

    Serial.println(); // Print a newline character
    Serial.println("STATE: Starting sequence cycle");
    gen_next_seq();
    Serial.print("ELAPSE: SEQUENCE_CYCLE ");
    Serial.println(formatWithCommas(elapsed));
  ...
  case WAITING_PHASE:
    ...
    {
      Serial.print("ELAPSED: WAITING PHASE ");
      Serial.println(formatWithCommas(elapsed));
